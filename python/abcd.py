...
with mp_hands.Hands(
    ...
    x_buffer = []
    buffer_size = 10
    threshold = 10  # threshold for detecting hand movement
    hand_detected = False  # flag variable to track whether hand has been detected or not
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if not hand_detected:
                    x, y, z = 0, 0, 0  # reset coordinates to (0,0,0)
                    hand_detected = True

        # If hand is not detected, print x, y, and z as 0
        else:
            x, y, z = 0, 0, 0
            hand_detected = False  # reset flag variable
            print(" 0,0,0")
            # Send hand position coordinates to other application using UDP communication
        sock.SendData(f"{x},{y}")
